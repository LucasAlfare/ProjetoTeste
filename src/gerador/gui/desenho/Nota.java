...
public class Nota {
    ...
    public Nota(int d, int dur) {                        // major diameter
        this.dur = dur;                           // n  means 1/n
        e = new Elipse(0.7*d, 0.5*d);            // shape
        mast = 2*d;
    }

    public void setNW(int x, int y) {                         // where
        this.x = x;
        this.y = y;
    }

    private void drawEighth(Graphics g) {                      // drawing
        drawQuarter(g);
    }

    private void drawMast(Graphics g) {
    ...
    private void drawQuarter(Graphics g) {
        int dx = (int)e.getMajor();
